...
#     elif _bkg_type == 'qcd_and_ttbar':
#         #bkgFileName =  dataDir + 'QCD_HT_ALL_AND_TTToHadronic_and_SemiLeptonic_1pb_weighted_Training_Jet2Xbb0p8Skim.root'
#         bkgFileName =  dataDir + 'QCD_HT_ALL_AND_TTToHadronic_and_SemiLeptonic_1pb_weighted_Mass30Skim_Training_BDTs.root'
...
#min_weight_bkg     = 1.0e-05 #[SX]: hardcode this for now
...
signal_sum_weight = 0
min_weight_signal = 9999999
for event_signal in df_signal.to_numpy():
    signal_sum_weight += event_signal[-1]
    if (np.absolute(event_signal[-1]) < min_weight_signal):
        min_weight_signal = np.absolute(event_signal[-1])

#min_weight_signal     = np.amin(df_signal.to_numpy()[:,-1])#find minimum weight
#min_weight_signal     = 1.0e-07 #[SX]: hardcode this for now
