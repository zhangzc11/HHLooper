...
inputBase=/eos/cms/store/group/phys_susy/razor/Run2Analysis/HHBoost/ntuple_v1p0/
 ...
 (set -x ;./HHLooper ${inputBase}ggH_amcnlo/ ggH.root ${TAG} 0 1 >&1) &
 (set -x ;./HHLooper ${inputBase}VBFH/ VBFH.root ${TAG} 0 1 >&1) &
 (set -x ;./HHLooper ${inputBase}WH/ WH.root ${TAG} 0 1 >&1) &
 (set -x ;./HHLooper ${inputBase}ZH/ ZH.root ${TAG} 0 1 >&1) &
...
hadd -k -f hists/${TAG}/singleH_skim.root hists/${TAG}/*H_skim.root
hadd -k -f hists/${TAG}/bkg_skim.root hists/${TAG}/qcd_skim.root hists/${TAG}/singleH_skim.root
